...
uint32_t exxtract_segment_32bits(uint32_t data, uint8_t startBit, uint8_t endBit)
{
   if(endBit < 32 && startBit <= endBit){
        uint32_t mask = ((1 << (endBit - startBit + 1)) - 1) << startBit; //Se llena la mascara de unos desde star hasta end, y las demas posiciones seran 0
        return (data & mask) >> startBit; /*Con la operacion AND se eliminan los bits que no estan en el rango deseado, como mask tiene unos en el segmento de posiciones que se desean guardar y cero en las demas posiciones, la operacion ADN mantiene solo los bits que se quieran guardar
...
uint64_t exxtract_segment_64bits(uint64_t data, uint8_t startBit, uint8_t endBit)
{
   if(endBit < 64 && startBit <= endBit){
        uint64_t mask = ((1 << (endBit - startBit + 1)) - 1) << startBit; //Se llena la mascara de unos desde star hasta end, y las demas posiciones seran 0
        return (data & mask) >> startBit; /*Con la operacion AND se eliminan los bits que no estan en el rango deseado, como mask tiene unos en el segmento de posiciones que se desean guardar y cero en las demas posiciones, la operacion ADN mantiene solo los bits que se quieran guardar
            Luego se desplazan el resultado del AND startBit veces, los bits que se caen se descartan y se colocan ceros*/
    } else{
        printf("\nRango de posiciones incorrecto\n");
...
uint16_t exxtract_ID(uint64_t data, uint8_t startBit, uint8_t endBit)
{
   if(endBit < 64 && startBit <= endBit){
        uint64_t mask = ((1 << (endBit - startBit + 1)) - 1) << startBit; //Se llena la mascara de unos desde star hasta end, y las demas posiciones seran 0
        return (data & mask) >> startBit; /*Con la operacion AND se eliminan los bits que no estan en el rango deseado, como mask tiene unos en el segmento de posiciones que se desean guardar y cero en las demas posiciones, la operacion ADN mantiene solo los bits que se quieran guardar
